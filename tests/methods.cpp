...
TEMPLATE_TEST_CASE("return_type_t", "[traits]", int, float, size_t) {
  ...
  CHECK(std::is_same_v<TestType,
                       poly::return_type_t<TestType(method1) noexcept>>);
  CHECK(std::is_same_v<TestType,
                       poly::return_type_t<TestType(method1) const noexcept>>);
  CHECK(std::is_same_v<TestType, poly::return_type_t<TestType(method1, int)>>);
  CHECK(std::is_same_v<TestType,
                       poly::return_type_t<TestType(method1, int) const>>);
  CHECK(std::is_same_v<TestType,
                       poly::return_type_t<TestType(method1, int) noexcept>>);
  CHECK(std::is_same_v<
        TestType,
        poly::return_type_t<TestType(method1, int) const noexcept>>);
}
TEMPLATE_TEST_CASE("method_name_t", "[traits]", int, float, size_t) {
  ...
  CHECK(
      std::is_same_v<method1, poly::method_name_t<TestType(method1) noexcept>>);
  CHECK(std::is_same_v<method1,
                       poly::method_name_t<TestType(method1) const noexcept>>);
  CHECK(std::is_same_v<method1, poly::method_name_t<TestType(method1, int)>>);
  CHECK(std::is_same_v<method1,
                       poly::method_name_t<TestType(method1, int) const>>);
  CHECK(std::is_same_v<method1,
                       poly::method_name_t<TestType(method1, int) noexcept>>);
  CHECK(std::is_same_v<
        method1,
        poly::method_name_t<TestType(method1, int) const noexcept>>);
}
