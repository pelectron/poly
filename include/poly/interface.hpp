...
public:
  ...
  // construct from other InterfaceTable featureing a super set of properties
  // and methods
  ...
  InterfaceTable(const InterfaceTable<Ps, Ms> &other)
      : InterfaceVTableEntry<MethodSpecs>(
        ...
        InterfacePTableEntry<PropertySpecs>(
            static_cast<const InterfacePTableEntry<PropertySpecs> &>(other))...,
        table_(other.table_) {}

  template <typename Ps, typename Ms>
  InterfaceTable(const object_table<Ps, Ms> *table)
      // note: aggregate initialization of base classes on purpose-> direclty
      // initialize offset
...
#define POLY_STRINGIFY1(x) #x
    static_assert(
        poly::detail::list_size<Ms>::value <= POLY_MAX_METHOD_COUNT,
    ...
    static_assert(
        poly::detail::list_size<Ps>::value <= POLY_MAX_PROPERTY_COUNT,
        "Error while creating an Interface from an Object: the number of "
        ...
        "(=" POLY_STRINGIFY(
            POLY_MAX_PROPERTY_COUNT) "). Adjust the POLY_MAX_PROPERTY_COUNT "
                                     "macro "
                                     "to reflect "
                                     "the maximum number of porperties "
                                     "accurately "
                                     "before "
                                     "including ANY poly header.");
...
public:
  ...
  template <typename Name>
  bool set(Name, const value_type_for<Name> &value) noexcept(is_nothrow<Name>) {
    static_assert(
