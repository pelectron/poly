...
struct filter_impl<List<T>, List<Us...>, Trait> {
  using type = std::conditional_t<Trait<T>::value, List<Us..., T>, List<Us...>>;
};
...
struct filter_impl<List<T, Ts...>, List<Us...>, Trait> {
  using list = std::conditional_t<Trait<T>::value, List<Us..., T>, List<Us...>>;
  using type = typename filter_impl<List<Ts...>, list, Trait>::type;
};

template <typename List, template <typename> typename Trait> struct filter;
template <template <typename...> typename List,
...
template <typename List> struct remove_duplicates;
template <template <typename...> typename List, typename... Ts>
...
/// Predicate<E>::value is true, E is added to the output, else E is discarded.
///
/// Example:
/// using L1 = type_list<char,const int,const double>;
...
///
/// @tparam List a TypeList
/// @tparam Predicate a unary meta function which must provide a static constexr
/// member called value.
...
/// check if T is a TypeList, that is T refers to a specialization of a template
/// class U, where U takes a template type parameter pack and no non-type
/// template arguments. In short: only true if T == U<Ts...>, where Ts... are
