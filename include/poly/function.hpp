...
namespace detail {
  ...
  struct invoke_ptr<Ret(Args...)> {
    ...
    using once_type = Ret (*)(Storage&, Args...);
  ...
  template<typename SigList, typename ArgList>
  ...
  struct resolve_signature<type_list<Sig, Sigs...>, type_list<Args...>> {
    using type = std::conditional_t<
        ...
        Sig,
        typename resolve_signature<type_list<Sigs...>,
                                   type_list<Args...>>::type>;
  };
  template<typename T>
  struct is_poly_function : std::false_type {};
  template<typename Sig, typename Derived>
  ...
  public:
    ...
    constexpr basic_function(
        const basic_function<Sig, OtherStorage>&
            other) noexcept(std::
                                is_nothrow_constructible_v<Storage,
                                                           const OtherStorage&>)
    ...
    constexpr basic_function(
        basic_function<Sig, OtherStorage>&&
    ...
    template<typename F>
    void bind(F&& f) noexcept(nothrow_emplacable<F>) {
...
public:
  ...
  template<typename Sig_, typename Storage>
  friend class once_function;
  template<typename F, typename = std::enable_if_t<not detail::is_poly_function<
                           std::decay_t<F>>::value>>
